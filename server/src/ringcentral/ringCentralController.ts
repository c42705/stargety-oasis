...
interface RingCentralParticipant {
  id: string;
  name: string;
  callId: string;
  joinedAt: Date;
...
const activeCalls: Map<string, RingCentralCall> = new Map();
const callParticipants: Map<string, RingCentralParticipant[]> = new Map();
...
export class RingCentralController {
  ...
  handleCallStarted(socket: Socket, data: { callId: string; hostId: string; topic: string }) {
    const { callId, hostId, topic } = data;

    const call: RingCentralCall = {
      id: callId,
      hostId,
      participants: [hostId],
      startTime: new Date(),
  ...
  handleParticipantJoined(socket: Socket, data: { callId: string; participantId: string; participantName: string }) {
    ...
    const participant: RingCentralParticipant = {
      id: participantId,
  ...
  // Handle call ended
  handleCallEnded(socket: Socket, data: { callId: string }) {
    const { callId } = data;
    this.endCall(callId);
  }
  ...
  handleParticipantLeft(socket: Socket, data: { callId: string; participantId: string }) {
